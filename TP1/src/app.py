...
# ÉTAPE 2 (6e) : Initialisation Session State
if "points" not in st.session_state:
    st.session_state["points"] = []  # liste de tuples (x, y, label)
if "last_pred" not in st.session_state:
    st.session_state["last_pred"] = None  # dict résultats ou None
# Gestion changement d'image (Reset des points si on change d'image)
...
# Reset state si changement d'image
if img_name != st.session_state["selected_image"]:
...
# ÉTAPE 3 (6f) : Bloc UI "Guidage points"
st.subheader("Guidage (optionnel) : points FG/BG")
c1, c2, c3 = st.columns(3)
with c1:
    # On met le slider au centre par défaut
...
with col_btn2:
    if st.button("Réinitialiser points"):
        st.session_state["points"] = []
# Affichage debug points
st.write(f"Points actuels : {len(st.session_state['points'])}")

# ÉTAPE 4 (6g) : Prévisualisation BBox + Points
def draw_preview(image_rgb: np.ndarray, box_xyxy: np.ndarray, points):
    preview = image_rgb.copy()
    bgr = cv2.cvtColor(preview, cv2.COLOR_RGB2BGR)

    # Dessin BBox
    x_start, y_start, x_end, y_end = [int(v) for v in box_xyxy.tolist()]
    cv2.rectangle(bgr, (x_start, y_start), (x_end, y_end), color=(0, 255, 0), thickness=2)

    # Dessin Points
    for (x, y, lab) in points:
        # FG (1) = Vert, BG (0) = Rouge
        ...
        cv2.circle(bgr, (int(x), int(y)), radius=6, color=(0,0,0), thickness=1)

    return cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)

preview = draw_preview(img, box, st.session_state["points"])
st.image(preview, caption="Prévisualisation : bbox + points", use_container_width=True)

# Bonus simple : avertissement bbox très petite
if (x_max - x_min) < 10 or (y_max - y_min) < 10:
    st.warning("BBox très petite : essayez une bbox plus large.")

# ÉTAPE 5 (6h) : Bouton de segmentation
do_segment = st.button("Segmenter", type="primary")
if do_segment:
    predictor = get_predictor()

    pts = st.session_state["points"]
    # Préparation des inputs points pour SAM
    if len(pts) > 0:
    ...
    t0 = time.time()
    # Appel de la nouvelle fonction Multimask + Points
...
# ÉTAPE 6 (6i) : Affichage et Sélection du masque
lp = st.session_state["last_pred"]

# On vérifie qu'on a un résultat et qu'il correspond bien à l'image affichée
if lp is not None and lp["img_name"] == img_name:
    ...
    # Selectbox pour choisir lequel des 3 masques afficher
    # On met par défaut celui qui a le meilleur score
    default_idx = int(np.argmax(scores))
    
    # On crée une liste lisible pour le selectbox
    options = list(range(len(scores)))
    idx = st.selectbox(
        "Index du masque (classé par score)", 
        options, 
        index=default_idx,
        format_func=lambda i: f"Masque {i} (Score: {scores[i]:.3f})"
    )
    
    # Récupération du masque choisi
    mask = masks[int(idx)].astype(bool)
    score_choisi = float(scores[idx])
    ...
    with col_res1:
        st.image(overlay, caption=f"Masque sélectionné : {idx} | Score: {score_choisi:.3f}", use_container_width=True)
    
    with col_res2:
        st.write(f"**Temps calcul:** {lp['time_ms']:.1f} ms")
        st.metric("Score IoU", f"{score_choisi:.3f}")
        st.write(f"**Aire:** {m_area} px")
        st.write(f"**Périmètre:** {m_per:.1f} px")
        st.write(f"**BBox Masque:** {m_bbox}")
        
        st.write("---")
        if st.button("Sauvegarder cet overlay"):
            OUT_DIR.mkdir(parents=True, exist_ok=True)
            # Nom fichier unique avec index masque
